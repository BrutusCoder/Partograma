...
export class InitialSchema1771715200285 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "lcg"."users_role_enum" AS ENUM('ENFERMEIRO_OBSTETRA', 'MEDICO_OBSTETRA', 'SUPERVISOR', 'AUDITOR', 'ADMIN')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "lcg"."episodes_labour_onset_type_enum" AS ENUM('SPONTANEOUS', 'INDUCED', 'UNKNOWN')`,
    );
    await queryRunner.query(
      `CREATE TYPE "lcg"."episodes_rupture_status_enum" AS ENUM('INTACT', 'RUPTURED', 'UNKNOWN')`,
    );
    await queryRunner.query(
      `CREATE TYPE "lcg"."episodes_status_enum" AS ENUM('ACTIVE', 'COMPLETED', 'ABORTED', 'TRANSFERRED', 'OTHER')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "lcg"."lcg_forms_status_enum" AS ENUM('ACTIVE', 'COMPLETED', 'EXPIRED_12H')`,
    );
    await queryRunner.query(
      `CREATE TABLE "lcg_forms" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "updated_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "episode_id" uuid NOT NULL, "form_number" smallint NOT NULL DEFAULT '1', "is_continued_form" boolean NOT NULL DEFAULT false, "started_at" TIMESTAMP WITH TIME ZONE NOT NULL, "ended_at" TIMESTAMP WITH TIME ZONE, "status" "lcg"."lcg_forms_status_enum" NOT NULL DEFAULT 'ACTIVE', "created_by" uuid NOT NULL, CONSTRAINT "PK_f947e4ebe2afd4d2736c23b4b1e" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_lcg_forms_episode_id" ON "lcg_forms" ("episode_id") `,
    );
    await queryRunner.query(`CREATE INDEX "IDX_lcg_forms_status" ON "lcg_forms" ("status") `);
    await queryRunner.query(
      `ALTER TABLE "users" ADD CONSTRAINT "FK_b94ecc6be926a5d23aa7791ec8a" FOREIGN KEY ("unit_id") REFERENCES "units"("id") ON DELETE SET NULL ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "episodes" ADD CONSTRAINT "FK_8cb9ff812751cf1bc59886b3b71" FOREIGN KEY ("created_by") REFERENCES "users"("id") ON DELETE RESTRICT ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "lcg_forms" ADD CONSTRAINT "FK_ac25781e4ec8b0f244bf0cf7e77" FOREIGN KEY ("episode_id") REFERENCES "episodes"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "lcg_forms" ADD CONSTRAINT "FK_d4df8df3027263363582d6c3384" FOREIGN KEY ("created_by") REFERENCES "users"("id") ON DELETE RESTRICT ON UPDATE NO ACTION`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "lcg_forms" DROP CONSTRAINT "FK_d4df8df3027263363582d6c3384"`,
    );
    await queryRunner.query(
      `ALTER TABLE "lcg_forms" DROP CONSTRAINT "FK_ac25781e4ec8b0f244bf0cf7e77"`,
    );
    await queryRunner.query(
      `ALTER TABLE "episodes" DROP CONSTRAINT "FK_8cb9ff812751cf1bc59886b3b71"`,
    );
